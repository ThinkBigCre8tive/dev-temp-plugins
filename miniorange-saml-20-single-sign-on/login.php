...
class saml_mo_login {
	...
	function plugin_settings_style( $page) {
        ...
        wp_enqueue_style('mo_saml_jquery_ui_style',plugins_url('includes/css/jquery-ui.min.css', __FILE__), array(), '4.8.96', 'all');
        wp_enqueue_style( 'mo_saml_admin_settings_style', plugins_url( 'includes/css/style_settings.min.css', __FILE__ ), array(), '4.8.96', 'all' );
        wp_enqueue_style( 'mo_saml_admin_settings_phone_style', plugins_url( 'includes/css/phone.css', __FILE__ ) );
        wp_enqueue_style( 'mo_saml_time_settings_style', plugins_url( 'includes/css/datetime-style-settings.min.css', __FILE__ ), array(),'4.8.96', 'all' );
			...
			if ( self::mo_check_option_admin_referer("mo_saml_verify_customer") ) {    //register the admin to miniOrange
                ...
                if($call_setup == false) {
                    ...
                    if ( $this->mo_saml_check_empty_or_null( $email )) {
                        update_option('mo_saml_message', 'Please fill up Email fields to submit your query.');
                        $this->mo_saml_show_error_message();
                        ...
                        update_option('mo_saml_message', 'Please fill up Schedule Call Details to submit your query.');
                        $this->mo_saml_show_error_message();
                    }
                    else if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                        update_option( 'mo_saml_message', 'Please enter a valid email address.' );
                        ...
                        if(!is_null($response)){
                            if ( $response == false ) {
                                ...
                                update_option('mo_saml_message', 'Thanks for getting in touch! You will receive the call details on your email shortly.');
                                $this->mo_saml_show_success_message();
                            }
                        }
                    }
